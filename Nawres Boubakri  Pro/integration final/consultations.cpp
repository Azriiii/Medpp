    ...
    ui(new Ui::Consultations)
{
    ...
    ui->diagnostique->addItem("rougeole");
    ui->diagnostique->addItem("grippe");
    ui->diagnostique->addItem("anemie");
    ui->diagnostique->addItem("diabete");
    ...
    model_2->setQuery("select REFF_CONST from CONSULTATION ")  ;
    ui->supprimer->setModel(model_2) ;




    ui-> TabConsultation->setModel(tmpconsultation.afficher());
    ui->TabConsultation->setItemDelegateForColumn(7, new NumberFormatDelegate(this));
    ui->TabConsultation->resizeColumnsToContents();
    ui->TabConsultation->setVisible(true);
   // ui->tableView_r->setModel(tmpc.afficher());

    qApp->setStyleSheet("QLineEdit { background-color: pink }QTextEdit { background-color: lightgray } QComboBox { background-color: gray } QDateEdit { background-color: lightgray }");
        ...
        QPropertyAnimation *animation1 = new QPropertyAnimation(ui->lc1, "geometry");
            ...
            QPropertyAnimation *animation2 = new QPropertyAnimation(ui->lc_2, "geometry");
                ...
                QPropertyAnimation *animation3 = new QPropertyAnimation(ui->lc_3, "geometry");
                    ...
                    ui->line_reff->setValidator( new QRegExpValidator(QRegExp("[0-9]{4}"),this) );
                    ui->line_exa->setValidator( new QRegExpValidator(QRegExp("[a-z-A-Z_]+"),this) );
                    ui->line_res->setValidator( new QRegExpValidator(QRegExp("[a-z-A-Z_]+"),this) );
                    ui->line_sym->setValidator( new QRegExpValidator(QRegExp("[a-z-A-Z_]+"),this) );

                    ui->exam->setValidator( new QRegExpValidator(QRegExp("[a-z-A-Z_]+"),this) );
                    ui->res_exam->setValidator( new QRegExpValidator(QRegExp("[a-z-A-Z_]+"),this) );
                    ui->symp->setValidator( new QRegExpValidator(QRegExp("[a-z-A-Z_]+"),this) );
                    ui->diag->setValidator( new QRegExpValidator(QRegExp("[a-z-A-Z_]+"),this) );
...
void Consultations::on_Ajouter_clicked()
{
   ...
   // QString diag=ui->line_dia->text();
    QString diag=ui->diagnostique->currentText();
...
void Consultations::on_modifier_consultation_clicked()
{
    QDate date_const;
...
void Consultations::on_supprimer_consultation_clicked()
{


    //int reff = ui->reff->text().toInt();
   int reff=ui->supprimer->currentText().toInt();
    ...
    {ui->TabConsultation->setModel(tmpconsultation.afficher());//refresh
         ...
         QSqlQueryModel * model= new QSqlQueryModel();
         model->setQuery("select REFF_CONST from CONSULTATION")  ;
...
void Consultations::on_rechercher_clicked()
{
    Consultation c;
     ...
     if(ui->reff_consultation->text().isEmpty())
     {
       ...
       //  c.recherche_consultation(ui->tableView_r,ui->reff_consultation->text().toInt());
        c.recherche_consultation(ui->TabConsultation,ui->reff_consultation->text().toInt());
   ...
   // c.recherche_consultation(ui->tableView_r,reff_const);
       c.recherche_consultation(ui->TabConsultation,reff_const);
...
void Consultations::on_sauvegarder_clicked()
{
...
void Consultations::on_imprimer_clicked()
{
...
void Consultations::on_tri_date_clicked()
{
    Consultation c;
...
void Consultations::on_tri_reff_clicked()
{
    Consultation c;
...
void Consultations::on_tri_id_clicked()
{
    Consultation c;
ui->TabConsultation->setModel(c.tri_reff_id()) ;
}

void Consultations::on_actualiser_clicked()
{
    QSqlQueryModel * model= new QSqlQueryModel();
    model->setQuery("select REFF_CONST from CONSULTATION")  ;
    ui->comboBox_3->setModel(model) ;
...
void Consultations::on_choisir_clicked()
{

    QString val=ui->comboBox_3->currentText()  ;
    QSqlQuery query ;
    query.prepare("select * from CONSULTATION where REFF_CONST='"+val+"'") ;

  if (query.exec())
  {
      while (query.next()) {

        ui->reff_const->setText(query.value(0).toString()) ;

        ui->idd->setText(query.value(6).toString()) ;

        ui->exam->setText(query.value(4).toString()) ;
        ui->diag->setText(query.value(3).toString()) ;
        ui->symp->setText(query.value(2).toString()) ;
        ui->dateEdit_2->setDate(query.value(1).toDate()) ;
        ui->res_exam->setText(query.value(5).toString()) ;
